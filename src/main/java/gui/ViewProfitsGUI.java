...
public class ViewProfitsGUI extends JFrame {
	...
	private JButton btnViewProfits = new JButton(ResourceBundle.getBundle("Etiquetas").getString("viewProfits")); //$NON-NLS-1$ //$NON-NLS-2$
	...
	private JRadioButton allProfits = new JRadioButton(ResourceBundle.getBundle("Etiquetas").getString("allProfits")); //$NON-NLS-1$ //$NON-NLS-2$
	private JLabel lblNewLabel = new JLabel(ResourceBundle.getBundle("Etiquetas").getString("NoBets")); //$NON-NLS-1$ //$NON-NLS-2$
	private String[] columnNamesEvents = new String[] { ResourceBundle.getBundle("Etiquetas").getString("Event"),
			ResourceBundle.getBundle("Etiquetas").getString("Query"),
			ResourceBundle.getBundle("Etiquetas").getString("Prediction"),
			ResourceBundle.getBundle("Etiquetas").getString("Bet"),
			ResourceBundle.getBundle("Etiquetas").getString("Profit"),
	 ...
	 * Create the frame.
	 */
	public ViewProfitsGUI(JFrame previousFrame, RegisteredClient logged) {
		try {
	...
	private void jbInit(JFrame previousFrame, RegisteredClient logged) throws Exception {
		...
		this.setTitle(ResourceBundle.getBundle("Etiquetas").getString("viewProfits"));
		
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		setBounds(100, 100, 960, 300);
		...
		Fromtext.setBounds(10, 28, 46, 14);
			...
			month2.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					day2.setModel(days2);
					btnViewProfits.setEnabled(true);
				}
			});
			getContentPane().add(month2);
			
			btnViewProfits.setBounds(102, 199, 131, 23);
			
			btnViewProfits.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					...
					betsCollection=facade.viewProfits(logged_rc, date1, date2);
					System.out.println("betssize"+betsCollection.size());
					allProfits.setEnabled(false);
					int kont=0;
						...
						for(Prediction p: bet.getPredictions()) {
							if(bet.getPredictions().size()==1) {
								if((date1.before(date2) || date1.equals(date2)) && (p.getQuestion().getEvent().getEventDate().after(date1) || p.getQuestion().getEvent().getEventDate().equals(date1)) && (p.getQuestion().getEvent().getEventDate().before(date2)|| p.getQuestion().getEvent().getEventDate().equals(date2))) {						
									...
									row.add(bet.getBet().getMoney()*p.getFee());
									row.add("-");
					...
					allProfits.setEnabled(true);
					allProfits.setSelected(false);
				}
			}
		});
			getContentPane().add(lblNewLabel);
			getContentPane().add(btnViewProfits);
			
	        scrollPane.setBounds(266, 53, 667, 169);
			...
			allProfits.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					if(allProfits.isSelected()==true) {
					tableModel.setDataVector(null, columnNamesEvents);
					
					BLFacade facade = MainGUI.getBusinessLogic();
					betsCollection=facade.viewAllProfits(logged_rc);
			...
			allProfits.setBounds(10, 158, 223, 23);
			getContentPane().add(allProfits);
