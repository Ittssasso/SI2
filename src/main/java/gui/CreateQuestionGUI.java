...
public class CreateQuestionGUI extends JFrame {
	...
	private final JLabel lblNewLabel = new JLabel(ResourceBundle.getBundle("Etiquetas").getString("CreateQuestionGUI.lblNewLabel.text")); //$NON-NLS-1$ //$NON-NLS-2$

	public CreateQuestionGUI(Vector<domain.Event> v, JFrame previousFrame) {
		try {
			jbInit(v, previousFrame);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}

	private void jbInit(Vector<domain.Event> v, JFrame previousFrame) throws Exception {
		...
		jLabelListOfEvents.setBounds(new Rectangle(275, 22, 277, 20));
		...
		jLabelEventDate.setBounds(40, 25, 140, 14);
