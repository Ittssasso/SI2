...
public class ObjectdbManagerServer extends JDialog {
	...
	public ObjectdbManagerServer() {
		...
		ConfigXML cc=ConfigXML.getInstance();
		
		if (cc.isDatabaseLocal()) {
		...
		try{
			...
			try {
		    	Runtime.getRuntime().exec("java -cp "+objectDbpath+" com.objectdb.Server -port "+ cc.getDatabasePort()+" start");
		    } catch (Exception ioe) {
		    	System.out.println (ioe);
		    }

		    textArea.append("\nAccess granted to: "+cc.getUser());
